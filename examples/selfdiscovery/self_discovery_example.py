...
dspy.settings.configure(lm=turbo, trace=[], temperature=0.1)
dspy.settings.show_guidelines = False


class SelfDiscoverModule(dspy.Module):
    def __init__(self):
        super().__init__()
...
discover = SelfDiscoverModule()

result = discover(task_to_resolve="""Lisa has 10 apples. She gives 3 apples to her friend and then buys 5 more apples from the store. How many apples does Lisa have now?""", reasoning_modules=reasoning_modules)
print(result)
